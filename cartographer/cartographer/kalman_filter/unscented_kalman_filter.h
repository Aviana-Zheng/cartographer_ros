...
无损卡尔曼滤波又称无迹卡尔曼滤波（Unscented Kalman Filter，UKF），
是无损变换（Unscented Transform，UT）与标准卡尔曼滤波体系的结合，
通过无损变换变换使非线性系统方程适用于线性假设下的标准卡尔曼体系
*/
...
*/
// Returns the matrix square root of a symmetric(对称) positive semidefinite(半正定) matrix.
template <typename FloatType, int N>
Eigen::Matrix<FloatType, N, N> MatrixSqrt(
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix<FloatType, N, N>>
      adjoint_eigen_solver((A + A.transpose()) / 2.);   //A==A的转置，构造特征分解对象
  ...
  //A = B * B   https://blog.csdn.net/ABC_ORANGE/article/details/107563644
...
class UnscentedKalmanFilter {
  ...
  */
  explicit UnscentedKalmanFilter(
      ...
      std::function<StateType(const StateType& state, const StateType& delta)>  // 函数指针类型
          add_delta = [](const StateType& state,    //参数2 λ表达式 
                         const StateType& delta) { return state + delta; }, 
      std::function<StateType(const StateType& origin, const StateType& target)>
          compute_delta =    //参数3   λ表达式
  ...
  */
  void Predict(std::function<StateType(const StateType&)> g,
    ...
    std::vector<StateType> Y;   //需要计算的状态矩阵，N*1矩阵 放入vector容器中
    // 在需要对大量数据进行处理的时候就要使用reserve主动分配内存以提升程序执行效率
    Y.reserve(2 * N + 1);    //公式：p65,  2N + 1个变量
    Y.emplace_back(g(mu));    //状态转移方程,公式p65:3.68,p70:3， 第0个变量
    /*
    可以看书，也可以参考https://blog.csdn.net/qq_34213260/article/details/108844092
    按照公式p65,3.66计算：
    1),mu是u,
    2),add_delta_()得到Xi,
    3),g(Xi)得到Yi,
    ...
    */
    ...
    // LOG(INFO) << "kSqrtNPlusLambda: " << kSqrtNPlusLambda << std::endl;
    // LOG(INFO) << "kMeanWeight0: " << kMeanWeight0 << std::endl;
    // LOG(INFO) << "kCovWeight0: " << kCovWeight0 << std::endl;
    // LOG(INFO) << "kCovWeightI: " << kCovWeightI << std::endl;
    for (int i = 0; i < N; ++i) { // 放入2 * N个变量
      // Order does not matter here as all have the same weights in the
      // summation later on anyways.
      // LOG(INFO) << g(add_delta_(mu, kSqrtNPlusLambda * sqrt_sigma.col(i)))[0] << ", "
      // << g(add_delta_(mu, kSqrtNPlusLambda * sqrt_sigma.col(i)))[1] << std::endl;
    ...
    for (int i = 0; i < N; ++i) {
      // kCovWeightI 乘以矩阵中的每一个数
       new_sigma += kCovWeightI * OuterProduct<FloatType, N>(
                                     compute_delta_(new_mu, Y[2 * i + 1]));
      // LOG(INFO) << new_sigma(0,0) << ", " 
      //         << new_sigma(0,1) << std::endl;
      // LOG(INFO) << new_sigma(1,0) << ", "
      //         << new_sigma(1,1) << std::endl;
      new_sigma += kCovWeightI * OuterProduct<FloatType, N>(
  ...
  6),...
  */
  template <int K>   // 参考https://zhuanlan.zhihu.com/p/41767489
  void Observe(
      std::function<Eigen::Matrix<FloatType, K, 1>(const StateType&)> h,
      const GaussianDistribution<FloatType, K>& delta) { // delta 是噪声
    ...
    CheckSymmetric(S);
    S += delta.GetCovariance();  // 加上噪声，新的传感器不确定性
    ...
    // 参考https://zhuanlan.zhihu.com/p/41767489
    const Eigen::Matrix<FloatType, N, K> kalman_gain =
        sigma_bar_xz * S.inverse();
    // LOG(INFO) << kalman_gain(0,1) << ", " << kalman_gain(1,0) << std::endl;
