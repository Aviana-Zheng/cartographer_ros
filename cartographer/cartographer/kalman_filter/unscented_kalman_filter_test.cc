...
*/
  for (int i = 0; i < 3; ++i) { //500
    ...
    filter.Predict(
        g, GaussianDistribution<double, 2>(Eigen::Vector2d(0., 0.),
                                           Eigen::Matrix2d::Identity() * 1e-9));
    LOG(INFO) << filter.GetBelief().GetCovariance()(0,0) << ", " 
              << filter.GetBelief().GetCovariance()(0,1) << std::endl;
    LOG(INFO) << filter.GetBelief().GetCovariance()(1,0) << ", "
              << filter.GetBelief().GetCovariance()(1,1) << std::endl;
    //测量/观察。传感器噪声的均值和方差是0
    filter.Observe<1>(
